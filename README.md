# Trading-Volatility




This project is involved in explaining and testing some of the trading strategies listed in (Z. Kakushadze and J.A. Serur. 151 Trading Strategies). I have always been interested in the idea of non directional trading, having zero exposure to the underlying. The strategies I focus on here involve trading volaltity, this means taking a position on future volatility,

the strategies I address are:
- Volatility trading with variance swaps
- Volatility risk premium trading, gamma hedging


 ## [Variance Swaps](<Variance_swap_trading/variance_swap.md>)  

The thinking behind this backtest is that the VIX methodology is similar, using a weighted strip of OTM options to come up with a 'fair strike'. 



 ## [Trading VRP with gamma hedging](<VRP_gamma_hedging/Introduction_to_GH.md>)  

 I am currently working on MIT OCW Topics In Mathematics With applications In Finance, to teach me the level of probability and stochastic calculas required to work around this topic. I will update this page when I start my research, into this 